...
fn main() {
    ...
    for row in 0..height {
        for col in 0..width {
            ...
            println!("Processing 'A' at [{}, {}]...", row, col);

            let neighbours = check_cell_neighbours(&cell, vec!['M', 'S'], Direction::Corners);
            let output_string = String::from_iter(neighbours.into_iter().map(|x| x.0.data));
            
            if output_string == "MSMS" || output_string == "SMSM" || output_string == "MMSS" || output_string == "SSMM" {
...
fn check_cell_neighbours<'a>(cell: &'a Cell<'a>, targets: Vec<char>, direction: Direction) -> Vec<(Cell<'a>, Direction)> {
...
impl<'a> Cell<'a> {
    ...
    fn neighbours(&self, direction: Direction) -> Neighbours {
        ...
        // Top Left
        if (matches!(direction, Direction::All) || (matches!(direction, Direction::Corners) || matches!(direction, Direction::TopLeft)) && (self.idx > self.map.row_width) && (self.idx > 0 && self.idx % self.map.row_width > 0)) {
        ...
        // Top Right
        if (matches!(direction, Direction::All) || (matches!(direction, Direction::Corners) || matches!(direction, Direction::TopRight)) &&(self.idx > self.map.row_width) && (self.idx % self.map.row_width < self.map.row_width - 1)) {
        ...
        // Bottom Left
        if (matches!(direction, Direction::All) || (matches!(direction, Direction::Corners) || matches!(direction, Direction::BottomLeft)) &&(self.idx + self.map.row_width < self.map.data.len()) && (self.idx > 0 && self.idx % self.map.row_width > 0)) {
        ...
        // Bottom Right
        if (matches!(direction, Direction::All) || (matches!(direction, Direction::Corners) || matches!(direction, Direction::BottomRight)) &&(self.idx + self.map.row_width < self.map.data.len()) && (self.idx % self.map.row_width < self.map.row_width - 1)) {
