...
fn process_map(map: &Map, width: i32, height: i32) -> usize {
    ...
    for frequency in antennas {
        // Every antenna is now also an antinode if we have more than one of them
        // if frequency.1.len() > 1 {
        //     for antenna in frequency.1 {
        //         unique_antinodes.insert(antenna);
        //     }
        // }

        let antinodes = process_frequency(&frequency.1, width, height);
...
fn process_frequency(antennas: &Vec<(i32, i32)>, width: i32, height: i32) -> HashSet<(i32, i32)> {
    let mut antinodes: HashSet<(i32, i32)> = HashSet::new();

    for (idx, a) in antennas[0..antennas.len() - 1].iter().enumerate() {
        for b in antennas[idx + 1..antennas.len()].iter() {
            let result = get_antinodes(a, b, width, height);

            for antinode in result {
...
fn get_antinodes(a: &(i32, i32), b: &(i32, i32), width: i32, height: i32) -> Vec<(i32, i32)> {
    //println!("Getting antinodes for [{},{}] and [{},{}]...", a.0, a.1, b.0, b.1);

    let mut antinodes: Vec<(i32, i32)> = Vec::new();

    // Antenna themselves are antinodes
    antinodes.push(*a);
    antinodes.push(*b);

    // Get the differences
    let mut row_diff = a.0 - b.0;
    let mut col_diff = a.1 - b.1;

    while a.0 + row_diff >= 0 && a.0 + row_diff < height && a.1 + col_diff >= 0 && a.1 + col_diff < width {
        let c = (a.0 + row_diff, a.1 + col_diff);
        antinodes.push(c);
        row_diff += a.0 - b.0;
        col_diff += a.1 - b.1;
    }
    ...
    row_diff = a.0 - b.0;
    col_diff = a.1 - b.1;
    while b.0 - row_diff >= 0 && b.0 - row_diff < height && b.1 - col_diff >= 0 && b.1 - col_diff < width {
        let d = (b.0 - row_diff, b.1 - col_diff);
        antinodes.push(d);
        row_diff += a.0 - b.0;
        col_diff += a.1 - b.1;
    }
