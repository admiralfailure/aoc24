...
fn process_map(map: &Map, width: usize, height: usize) -> usize {
    ...
    for row in 0..height {
        for col in 0..width {
            ...
            let visited_end_positions = process_cell(&map, &cell);
            let trailhead_score = visited_end_positions.len();
...
fn process_cell(map: &Map, cell: &Cell) -> HashSet<(usize, usize)> {
    let mut visited_end_positions: HashSet<(usize, usize)> = HashSet::new();
    ...
    for neighbour in cell.neighbours(&Direction::Square).all() {
        ...
        if neighbour_value == cell_value + 1 {
            ...
            if neighbour_value == 9 {
                visited_end_positions.insert((neighbour.row(), neighbour.col()));
            }
            else {
                // Else, recurse down this path
                let others = process_cell(&map, &neighbour);
                for other in others {
                    visited_end_positions.insert(other);
                }
            }
        }
    }

    return visited_end_positions;
}
